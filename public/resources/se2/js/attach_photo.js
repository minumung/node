    ...
    var date_s   = date.getSeconds();
    if (("" + month).length == 1) month = "0" + month;
    if (("" + day).length   == 1) day   = "0" + day;
    if (("" + date_h).length == 1) date_h = "0" + date_h;
    if (("" + date_m).length   == 1) date_m   = "0" + date_m;
    if (("" + date_s).length   == 1) date_s   = "0" + date_s;
    var todate = year+""+month+""+day+""+date_h+""+date_m+""+date_s;
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src","/resources/se2/img/photoQuickPopup/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src","/resources/se2/img/photoQuickPopup/btn_confirm.png");
    	 }
     }   
    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="/resources/se2/img/photoQuickPopup/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {
    	...
    	sUploadURL= '/api/file/upload_binary'; 	//upload URL
    	//파일을 하나씩 보내고, 결과를 받음.
    	for(var j=0, k=0; j < nImageInfoCnt; j++) {
    		tempFile = htImageInfo['img'+j];
    		try{
	    		if(!!tempFile){
	    			callAjaxForHTML5(tempFile,sUploadURL, function(filename){
	    				opener.pasteHTML(filename);
		    			k += 1;
	    			});
	    		}
	    	}catch(e){}
    	...
    	setTimeout(function(){
    		popClose(nImageInfoCnt, k);
    	},2000)
	}
    
    function popClose(o,x){
    	if(o === x) {
    		self.close();
    	}
    }
    function callAjaxForHTML5 (tempFile, sUploadURL, callback){
    	var oAjax = jindo.$Ajax(sUploadURL, {
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				if (res.readyState() == 4) {
					...
					callback(res._response.responseText)
				}else {
					alert("실패")
				}
			},
		...
		oAjax.header("file-name-date",todate);
...
// 	    		oCustomEvent.stop(); 수행시 bAllowed 가 false이더라도 alert이 수행되지 않음
	...
	window.onload = function(){
  		...
  		if(bSupportDragAndDropAPI){ //html5 일경우 ui 변경
  			$Element("btn_confirm").attr("onclick","#");//오혜성 html5 미지원여부에  form의 action을 다르게 해줘야한다. html5 지원하는 브라우저도 다른 url 타는것 같지만 결국 form의 액션을 타네
